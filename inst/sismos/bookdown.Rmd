...
# Gráfico combinado de profundidad y magnitud
combined_plot <- plot_ly() %>%
  add_trace(data = sismo_4_max_completo, x = ~FECHA, y = ~MIN_PROFUNDIDAD, text = ~paste("Magnitud: ", MAX_MAGNITUD_ML), type = 'scatter', mode = 'markers', name = 'Profundidad', marker = list(color = 'saddlebrown')) %>%
  layout(title = "Profundidad y Magnitud del sismo en el tiempo",
...
modelo_magnitud_arima <- auto.arima(indice.ts)
...
pronostico_magnitud_arima <- forecast::forecast(modelo_magnitud_arima, h=120)
...
```{r}
# Ajustar el modelo ARIMA
modelo_profundidad_arima <- auto.arima(P_indice.ts)
...
```{r}
# Hacer predicciones para los próximos 4 meses utilizando la función forecast del paquete forecast
pronostico_profundidad_arima <- forecast::forecast(modelo_profundidad_arima, h=120)
...
modelo_magnitud_prophet <- prophet(data_magnitud)
...
```
La gráfica muestra el pronóstico de la magnitud de los sismos en el municipio de los Santos, Santander en el tiempo utilizando el modelo de series temporales Prophet. En el eje horizontal se representa el tiempo desde el año 2011 hasta el 2018, y en el eje vertical se presenta la magnitud de los sismos. Los puntos negros corresponden a las magnitudes observadas de los sismos, mientras que la línea azul muestra la tendencia central pronosticada por el modelo. Las bandas de color azul claro alrededor de la línea central representan los intervalos de confianza del pronóstico, indicando la incertidumbre del modelo en sus predicciones. La gráfica sugiere que la magnitud de los sismos ha permanecido relativamente constante en el tiempo, con algunas fluctuaciones y valores atípicos ocasionales.


```{r}
# Evaluar los residuos
residuales_magnitud_prophet <- data$y - pronostico_magnitud_prophet$yhat[1:length(data$y)]

# Prueba de Shapiro-Wilk para normalidad
shapiro.test(residuales_magnitud_prophet)

# Prueba de Ljung-Box para autocorrelación
Box.test(residuales_magnitud_prophet, type = "Ljung-Box")
```
...
# Ajustar el modelo Prophet
modelo_profundidad_prophet <- prophet(data_profundidad)
...
plot(modelo_profundidad_prophet, pronostico_profundidad_prophet) +
  labs(title = "Pronóstico de Profundidad de Sismos", y = "Profundidad", x = "Tiempo")
```
...
```{r}
# Evaluar los residuos
residuales_profunidad_prophet <- data$y - pronostico_profundidad_prophet$yhat[1:length(data$y)]
...
modelo_regresion <- lm(y ~ ds, data = data)

# Predicciones de regresión lineal
data$pred_regresion <- predict(modelo_regresion, newdata = data)

# Evaluar los residuos del modelo de regresión
residuales_regresion <- data$y - data$pred_regresion

# Prueba de Shapiro-Wilk para normalidad de los residuos de la regresión
shapiro_regresion <- shapiro.test(residuales_regresion)

# Prueba de Ljung-Box para autocorrelación de los residuos de la regresión
box_regresion <- Box.test(residuales_regresion, type = "Ljung-Box")

# Resultados
print("Resultados del modelo Prophet:")
print(shapiro_prophet)
print(box_prophet)

print("Resultados del modelo de regresión:")
print(shapiro_regresion)
```
```{r}
...
```{r}
head(data)
head(pronostico_profundidad)
head(pronostico_prophet)
head(pronostico_profundidad$yhat)
head(data$ds)
head(data$y)
head(future)
# Graficar las predicciones de ambos modelos para comparación
plot(data$ds, data$y, main = "Comparación de Predicciones de Modelos", ylab = "Profundidad", xlab = "Tiempo", col = "black", pch = 16)
lines(future$ds, pronostico_prophet$yhat, col = "blue", lwd = 2, lty = 1)
lines(modelo_profundidad$ds, modelo_profundidad$y, col = "red", lwd = 2, lty = 2)
legend("topleft", legend = c("Datos Observados", "Pronóstico Prophet", "Pronóstico Regresión"), col = c("black", "blue", "red"), lty = c(NA, 1, 2), pch = c(16, NA, NA))
```
