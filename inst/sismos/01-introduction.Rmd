...
modelo_magnitud <- auto.arima(indice.ts)

# Resumen del modelo
summary(modelo_magnitud)
```
...
pronostico_magnitud <- forecast::forecast(modelo_magnitud, h=120)

# Graficar las predicciones
plot(pronostico_magnitud, main="Pronóstico de Magnitud de Sismos", ylab="Magnitud", xlab="Tiempo")
...
```{r}
# Residuos del modelo
residuales <- residuals(modelo_magnitud)
...
```{r}
# Ajustar el modelo ARIMA
modelo_profundidad <- auto.arima(P_indice.ts)

# Resumen del modelo
summary(modelo_profundidad)
```
...
```{r}
# Hacer predicciones para los próximos 4 meses utilizando la función forecast del paquete forecast
pronostico_profundidad <- forecast::forecast(modelo_profundidad, h=120)

# Graficar las predicciones
plot(pronostico_profundidad, main="Pronóstico de Profundidad de Sismos", ylab="Profundidad", xlab="Tiempo")
```
La gráfica presenta el pronóstico de la profundidad de los sismos para los próximos 4 meses basado en un modelo ARIMA ajustado a los datos históricos desde 2011 hasta 2018. La consistencia entre las predicciones y los datos históricos sugiere que el modelo ha capturado bien los patrones de la serie temporal y los intervalos de confianza indican una mayor certeza en las predicciones a corto plazo y mayor incertidumbre en el largo plazo.

```{r}
# Residuos del modelo
residuales <- residuals(modelo_profundidad)
...
modelo_profundidad <- prophet(data)

# Hacer predicciones para los próximos 4 meses (120 días)
future <- make_future_dataframe(modelo_profundidad, periods = 120)
pronostico_profundidad <- predict(modelo_profundidad, future)

# Graficar las predicciones
plot(modelo_profundidad, pronostico_profundidad) +
  labs(title = "Pronóstico de Profundidad de Sismos", y = "Profundidad", x = "Tiempo")

# Evaluar los residuos
residuales <- data$y - pronostico_profundidad$yhat[1:length(data$y)]
